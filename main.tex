...
\begin{figure}[h!]
	\subfigure[Car]{
		\begin{minipage}[b]{.3\linewidth}
			\centering
			\includegraphics[scale=0.55]{./MCCP_results/car.pdf}
		\end{minipage}
	}
	\subfigure[Flower]{
		\begin{minipage}[b]{.3\linewidth}
			\centering
			\includegraphics[scale=0.55]{./MCCP_results/flower.pdf}
		\end{minipage}
	}
	\subfigure[Food]{
		\begin{minipage}[b]{.3\linewidth}
			\centering
			\includegraphics[scale=0.55]{./MCCP_results/food.pdf}
		\end{minipage}
	}
	...
	\subfigure[Fruit]{
		\begin{minipage}[b]{.3\linewidth}
			\centering
			\includegraphics[scale=0.55]{./MCCP_results/fruit.pdf}
		\end{minipage}
	}
	\subfigure[Sport]{
		\begin{minipage}[b]{.3\linewidth}
			\centering
			\includegraphics[scale=0.55]{./MCCP_results/sport.pdf}
		\end{minipage}
	}
	\subfigure[Weather]{
		\begin{minipage}[b]{.3\linewidth}
			\centering
			\includegraphics[scale=0.55]{./MCCP_results/weather.pdf}
		\end{minipage}
	}
	...
	\subfigure[Animal\_1]{
		\begin{minipage}[b]{.3\linewidth}
			\centering
			\includegraphics[scale=0.55]{./MCCP_results/animal_1.pdf}
		\end{minipage}
	}
	\subfigure[Animal\_2]{
		\begin{minipage}[b]{.3\linewidth}
			\centering
			\includegraphics[scale=0.55]{./MCCP_results/animal_2.pdf}
		\end{minipage}
	}
	\subfigure[Animal\_3]{
		\begin{minipage}[b]{.3\linewidth}
			\centering
			\includegraphics[scale=0.55]{./MCCP_results/animal_3.pdf}
		\end{minipage}
	}
...
生成变异模型：
变异算子是变异测试的核心技术，用于变异原始深度学习模型。 最近，Shen等人提出了一种称为 MuNN\cite{shen2018munn} 的深度学习变异方法，由 5 个模型级变异算子组成。之后，Ma等人提出了一种更全面的深度学习变异技术，称为DeepMutation\cite{ma2018deepmutation}，它包含16个（即8个源级和8个模型级）深度学习变异算子。 在后门攻击场景中，源级变异算子不适合，因为我们只能获取后门注入的深度学习模型，而不能获取训练数据或训练代码。 具体来说，在本研究中，我们在 DeepMutation\cite{ma2018deepmutation}中采用了 5 个模型级 深度学习特定变异算子如下表\ref{Mutation_Operator}所示，表中第一列表示变异算子的名称，表中第二列表示算子作用域，表中第三列描述变异算子的作用。它们在语法上与 MuNN\cite{shen2018munn}中的变异算子相似。 没有考虑算子“Layer Deactivation(M)”和“Layer Addition(M)”，因为我们研究的深度学习模型不满足结构条件。 此外，我们进一步删除了算子“Activation Function Removal(M)”，因为该算子只能生成很少数量的变异体（与深度学习模型中的层数一致），这不适合我们的工作。
...
具体的实现算法如算法\ref{algo_BSD}所示。第1行，我们先预先定义从小到大的变异率。第2行至第10行，完成确定攻击目标类，即变异率从小到大对后门模型进行变异，当可以确定出攻击目标类时终止，不再进行变异否则继续变异。第12行至第19行，利用种子样本对变异模型进行排序。第21行至24行，计算待测样本熵，并生成样本的优先级队列。
...
\begin{figure}[h!]
	\subfigure[BadNets]{
		\begin{minipage}[b]{.3\linewidth}
			\centering
			\includegraphics[scale=0.28]{./Backdoor_detect_result/target_class/BadNets.png}
		\end{minipage}
	}
	\subfigure[Blended]{
		\begin{minipage}[b]{.3\linewidth}
			\centering
			\includegraphics[scale=0.28]{./Backdoor_detect_result/target_class/Blended.png}
		\end{minipage}
	}
	\subfigure[IAD]{
		\begin{minipage}[b]{.3\linewidth}
	...
	\subfigure[LabelConsistent]{
		\begin{minipage}[b]{.3\linewidth}
			\centering
			\includegraphics[scale=0.28]{./Backdoor_detect_result/target_class/LabelConsistent.png}
		\end{minipage}
	}
	\subfigure[Refool]{
		\begin{minipage}[b]{.3\linewidth}
			\centering
			\includegraphics[scale=0.28]{./Backdoor_detect_result/target_class/Refool.png}
		\end{minipage}
	}
	\subfigure[WaNet]{
		\begin{minipage}[b]{.3\linewidth}
...
\begin{figure}[h!]
	\subfigure[BadNets]{
		\begin{minipage}[b]{.3\linewidth}
			\centering
			\includegraphics[scale=0.28]{./Backdoor_detect_result/detect/BadNets.png}
		\end{minipage}
	}
	\subfigure[Blended]{
		\begin{minipage}[b]{.3\linewidth}
			\centering
			\includegraphics[scale=0.28]{./Backdoor_detect_result/detect/Blended.png}
		\end{minipage}
	}
	\subfigure[IAD]{
		\begin{minipage}[b]{.3\linewidth}
	...
	\subfigure[LabelConsistent]{
		\begin{minipage}[b]{.3\linewidth}
			\centering
			\includegraphics[scale=0.28]{./Backdoor_detect_result/detect/LabelConsistent.png}
		\end{minipage}
	}
	\subfigure[Refool]{
		\begin{minipage}[b]{.3\linewidth}
			\centering
			\includegraphics[scale=0.28]{./Backdoor_detect_result/detect/Refool.png}
		\end{minipage}
	}
	\subfigure[WaNet]{
		\begin{minipage}[b]{.3\linewidth}
